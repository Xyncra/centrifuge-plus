...
func (b *TopicBroker) Subscribe(ch string) error {
...
func (b *TopicBroker) Unsubscribe(ch string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (b *TopicBroker) PublishWithContext(ctx context.Context, ch string, data []byte, opts centrifuge.PublishOptions) (sp centrifuge.StreamPosition, fromCache bool, err error) {
	...
	if opts.IdempotencyKey != "" {
		...
		if err == nil && len(cached) >= 2 && cached[0] != "" {
			...
			if parseErr == nil {
				return centrifuge.StreamPosition{Offset: offset, Epoch: cached[0]}, true, nil
			}
		}
	}
	...
	if err != nil {
		return centrifuge.StreamPosition{}, false, fmt.Errorf("BatchIncrby failed: %w", err)
	}
	...
	if err := b.PublishWithOffset(ctx, ch, data, opts, position); err != nil {
		return centrifuge.StreamPosition{}, false, fmt.Errorf("PublishWithOffset failed: %w", err)
	}
