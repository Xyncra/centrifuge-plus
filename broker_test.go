...
func TestTopicBroker_Publish(t *testing.T) {
	...
	sp, fromCache, err := broker.Publish("test-channel", data, opts)
	if err != nil {
...
func TestTopicBroker_PublishIdempotent(t *testing.T) {
	...
	sp1, fromCache1, err := broker.Publish("test-channel", data, opts)
	if err != nil {
	...
	sp2, fromCache2, err := broker.Publish("test-channel", data, opts)
	if err != nil {
...
func TestTopicBroker_History(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		_, _, err := broker.Publish("test-channel", data, opts)
		if err != nil {
			t.Fatalf("publish failed: %v", err)
		}
	}
...
func TestTopicBroker_HistoryWithFilterSince(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		_, _, err := broker.Publish("filter-channel", data, opts)
		if err != nil {
...
func TestDualBroker_Routing(t *testing.T) {
	...
	liveSp, _, err := broker.Publish("live-channel", liveData, liveOpts)
	if err != nil {
	...
	topicSp, _, err := broker.Publish("topic-channel", topicData, topicOpts)
	if err != nil {
...
func TestTopicBroker_PublishMultipleChannels(t *testing.T) {
	...
	for _, ch := range channels {
		...
		sp, _, err := broker.Publish(ch, data, opts)
		if err != nil {
...
func TestTopicBroker_PublishEmptyData(t *testing.T) {
	...
	sp, _, err := broker.Publish("test-channel", data, opts)
	if err != nil {
...
func TestTopicBroker_PublishWithIdempotencyKeyTests(t *testing.T) {
	...
	sp1, _, err := broker.Publish("test-channel", data, opts1)
	if err != nil {
		t.Fatalf("first publish failed: %v", err)
	}
	...
	sp2, _, err := broker.Publish("test-channel", data, opts2)
	if err != nil {
		t.Fatalf("second publish failed: %v", err)
	}
...
func TestDualBroker_UnregisterChannelType(t *testing.T) {
	...
	// 未注册的 channel 应该返回错误
	sp, _, err := broker.Publish("unregistered-channel", data, opts)
	if err == nil {
...
func TestDualBroker_UnregisteredChannel_AllMethods(t *testing.T) {
	...
	_, _, err = broker.PublishWithContext(context.Background(), ch, []byte("{}"), centrifuge.PublishOptions{})
	if err == nil {
...
func TestTopicBroker_TableDrivenPublish(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			sp, fromCache, err := broker.Publish(tt.channel, tt.data, opts)
			if tt.wantErr {
				if err == nil {
...
func TestTopicBroker_DirtyDataIsolation(t *testing.T) {
	...
	sp1, _, err := broker.Publish("isolated-channel", data, opts)
	if err != nil {
	...
	sp2, _, err := broker.Publish("isolated-channel", data, opts)
	if err != nil {
...
func TestTopicBroker_PublishLargePayload(t *testing.T) {
	...
	sp, _, err := broker.Publish("large-channel", data, opts)
	if err != nil {
...
func TestDualBroker_ConcurrentRegisterAndPublish(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(idx int) {
			...
			_, _, _ = broker.Publish(ch, []byte(`{}`), opts)
...
func TestTopicBroker_History_MaxUint32Clamp(t *testing.T) {
	...
	// 发布一条消息以创建 meta key
	_, _, _ = broker.Publish("ch-1", []byte(`{}`), centrifuge.PublishOptions{})
