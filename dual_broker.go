...
// Unsubscribe unsubscribes node from channel.
// 注意：接口签名不含 context 参数（与 centrifuge.NodeBroker 保持一致），
// 此处 context.Background() 仅用于 tracing span 创建，不影响业务逻辑。
func (d *DualBroker) Unsubscribe(ch string) error {
	ct, err := d.getChannelType(ch)
	if err != nil {
		return err
	}
...
func (d *DualBroker) Publish(ch string, data []byte, opts centrifuge.PublishOptions) (centrifuge.StreamPosition, bool, error) {
...
func (d *DualBroker) PublishWithContext(ctx context.Context, ch string, data []byte, opts centrifuge.PublishOptions) (sp centrifuge.StreamPosition, fromCache bool, err error) {
	...
	switch ct {
	case Topic:
		sp, fromCache, err = d.topicBroker.PublishWithContext(ctx, ch, data, opts)
		return
	default: // Live
		// 已知限制：centrifuge.RedisBroker.Publish 不支持 context 参数（v0.38.0），
		// 内部使用 context.Background()，tracing 链路在此断裂。
		// 如果上游库后续版本添加 PublishWithContext，应优先使用。
		sp, fromCache, err = d.liveBroker.Publish(ch, data, opts)
		return
	}
}
...
func (d *DualBroker) PublishEphemeral(ctx context.Context, ch string, data []byte, opts centrifuge.PublishOptions) error {
	...
	_, _, err := d.liveBroker.Publish(ch, data, opts)
	recordError(span, err)
	return err
}
